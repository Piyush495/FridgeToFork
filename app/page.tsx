...
export default function LandingPage() {
  return (
    <main className="min-h-screen bg-[#FEFAE0] font-sans overflow-x-hidden">
      ...
      {/* NAV */}
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-[5%] py-5 bg-[#FEFAE0]/85 backdrop-blur-md border-b border-[#2D6A4F]/10">
        ...
        <div className="flex items-center gap-8">
          <a href="#how" className="text-sm text-[#7A7A6E] hover:text-[#2D6A4F] transition">How it works</a>
          <a href="#features" className="text-sm text-[#7A7A6E] hover:text-[#2D6A4F] transition">Features</a>
          <Link href="/register" className="bg-[#2D6A4F] text-white px-5 py-2.5 rounded-full text-sm font-medium hover:bg-[#1e5038] transition shadow-lg shadow-[#2D6A4F]/20">
            Get started free
          </Link>
        </div>
      </nav>
      ...
      <section className="min-h-screen flex flex-col items-center justify-center text-center px-[5%] pt-32 pb-20 relative">
        <div className="absolute inset-0 pointer-events-none">
          {["🥦", "🍳", "🧅", "🍅", "🥕", "🌿"].map((e, i) => (
            <span key={i} className="absolute text-4xl opacity-15 animate-bounce" style={{
        ...
        <div className="inline-flex items-center gap-2 bg-[#D8F3DC] text-[#2D6A4F] px-4 py-1.5 rounded-full text-xs font-medium mb-6 border border-[#52B788]/30">
          ✦ AI-Powered Meal Planning
        </div>

        <h1 className="font-serif text-6xl md:text-8xl font-black leading-[1.05] tracking-tight text-[#1B1B1B] max-w-4xl mb-6">
          What's in your fridge?<br />Let <em className="text-[#2D6A4F] not-italic">AI</em> decide dinner.
        </h1>

        <p className="text-lg text-[#7A7A6E] max-w-lg leading-relaxed font-light mb-10">
        ...
        <div className="flex gap-4 items-center flex-wrap justify-center">
          <Link href="/register" className="bg-[#2D6A4F] text-white px-8 py-3.5 rounded-full text-base font-medium hover:bg-[#1e5038] transition shadow-xl shadow-[#2D6A4F]/30 hover:-translate-y-0.5">
            Start cooking for free →
          </Link>
          <a href="#how" className="px-8 py-3.5 rounded-full text-base border border-black/10 hover:border-[#2D6A4F] hover:text-[#2D6A4F] transition">
            See how it works
          </a>
        </div>

        {/* Demo card */}
        <div className="mt-16 bg-white rounded-2xl p-6 shadow-2xl max-w-md w-full border border-black/5 text-left">
          ...
          <div className="bg-[#FEFAE0] rounded-xl p-4 border border-dashed border-[#2D6A4F]/20">
            <p className="font-serif text-lg font-bold text-[#1B1B1B] mb-1">Paneer Bhurji with Masala Eggs</p>
      ...
      <section id="how" className="px-[5%] py-24">
        ...
        <h2 className="font-serif text-5xl font-black text-center text-[#1B1B1B] tracking-tight mb-16">From fridge to fork<br />in three steps</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
          {[
            ...
            <div key={s.num} className="bg-white rounded-2xl p-8 border border-black/5 text-center hover:-translate-y-1 hover:shadow-xl transition-all">
      ...
      <section id="features" className="px-[5%] pb-24">
        ...
        <h2 className="font-serif text-5xl font-black text-center text-[#1B1B1B] tracking-tight mb-16">Everything you need<br />to eat well</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 max-w-4xl mx-auto">
          {[
            ...
            <div key={f.title} className="bg-white rounded-2xl p-6 border border-black/5">
      ...
      {/* CTA */}
      <div className="mx-[5%] mb-20 bg-[#2D6A4F] rounded-3xl px-[5%] py-20 text-center relative overflow-hidden">
        <h2 className="font-serif text-5xl font-black text-white tracking-tight mb-4">Ready to stop wasting food?</h2>
        <p className="text-white/70 text-lg mb-8 font-light">Turn your leftover ingredients into delicious meals today.</p>
        <Link href="/register" className="inline-block bg-white text-[#2D6A4F] px-10 py-3.5 rounded-full font-medium hover:-translate-y-0.5 hover:shadow-xl transition-all">
          Get started — it's free
        </Link>
      </div>

      {/* FOOTER */}
      <footer className="text-center pb-8 text-xs text-[#7A7A6E] border-t border-black/5 pt-6">
